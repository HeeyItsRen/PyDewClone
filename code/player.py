...
class Player(pygame.sprite.Sprite):
    def __init__(self, position, group, collision_sprites, tree_sprites, interaction, soil_layer, toggle_shop):
        ...
        self.z = LAYERS['main'] #Player has Z position 
    ...
    def animate(self, dt):
        ...
        # self.frame_index = (self.frame_index + dt * 4) % len(self.animations[self.status]) #Cleaner animation code but animates too fast
    ...
    def input(self):
        ...
        if not self.timers['tool use'].active and not self.sleep:
            ...
            if keys[pygame.K_RETURN]:
                collided_interaction_sprite = pygame.sprite.spritecollide(self, self.interaction, False) #False means if self collides with sprite do we want to destroy
    ...
    def get_status(self):
        #If player is not moving (idle movement):
    ...
    def move(self, dt):
        ...
        #Normalizing a vector for angle speed
        if self.direction.magnitude() > 0:
            self.direction = self.direction.normalize()
        
        #Horizontal direction
        self.pos.x += self.direction.x * self.speed * dt
        self.hitbox.centerx = round(self.pos.x) #Round or else py will truncate the number
        self.rect.centerx = self.hitbox.centerx
