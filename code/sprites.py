...
class Tree(Generic):
    def __init__(self, position, surface, groups, name, player_add):
        ...
        self.apple_position = APPLE_POS[name] #Name is either small or large
    ...
    def damage(self):
        ...
        #Remove an apple IF there is an apple
        if len(self.apple_sprites.sprites()) > 0:
            random_apple = choice(self.apple_sprites.sprites())
            Particle(position = random_apple.rect.topleft, 
    ...
    def check_death(self):
        if self.health <= 0:
            Particle(position = self.rect.topleft,
                     surface = self.image,
                     groups = self.groups()[0],
                     z = LAYERS['fruit'],
                     duration = 300)
